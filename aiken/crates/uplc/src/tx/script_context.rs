        ...
        RedeemerTag::Spend => get_tx_in_info_v2(&tx.transaction_body.inputs, utxos)
            ...
            .and_then(|input| match output_address(&input.resolved) {
                Address::Shelley(shelley_address) => {
                    println!("Shelley address: {:?}", shelley_address);
                    println!("Shelley address: {:?}", shelley_address.to_bech32());
                    println!(
                        "Shelley address payment as hash: {:?}",
                        shelley_address.payment().as_hash()
                    );
